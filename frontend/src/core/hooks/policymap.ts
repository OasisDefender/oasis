...
const fillLines = (args: {origLines: LineInfo, actualScheme: ChildrenInfo}): LineInfo => {
    ...
    function walkChildrens(children: ChildItem[], visible:boolean, visibleParent: ChildItem) {
        children.forEach((c) => {
            visibilityOfItems[c.id] = visible ? c : visibleParent;
            if (c.children) {
                walkChildrens(c.children, visible && !(c.childrenCollapsed ?? DEFAULT_COLLAPSED), visible ? c : visibleParent);
            }
        });
    }
    ...
    origLines.items.forEach((line) => {
        ...
        if (srcVisible && dstVisible) {
            ...
            if (srcVisibility !== dstVisibility) {
                // Not "internal" line
                const foundFoldedLine = foldedLines.find((l) => 
                ...
                if (foundFoldedLine) {
                    ...
                    foldedLines.push({
                        id: `gen_${srcVisibility.id}_${dstVisibility.id}_${line.type ?? ""}`,
...
export function usePolicyMap() {
    ...
    async function fetchData(classificationIds: number[]) {
        ...
        try {
            ...
            response.data.lines = fillLines({origLines: origLines.current, actualScheme: response.data.scheme});
    ...
    const toogleChildren = useCallback((id: string) => {
        setData((oldData) => {
            ...
            return {
                lines: fillLines({origLines: origLines.current, actualScheme: newData}),
                scheme: newData,
            };
        });
    }, []);
