...
export function Clouds() {
    ...
    const [addCloudModalOpened, { open: openAddCloudModal, close: closeAddCloudModal }] = useDisclosure(false);
    ...
    const makeSync = async (cloud: ICloudView) => {
        try {
            ...
            ShowModalError(`Cannot sync cloud "${cloud.name}"`, e as AxiosError);
        }
    };
    ...
    const makeDelete = async (cloud: ICloudView) => {
        try {
            ...
            ShowModalError(`Cannot delete cloud "${cloud.name}"`, e as AxiosError);
        }
    };
    ...
    const rows = clouds.map((cloud) => (
        <CloudTableRow key={cloud.id} cloud={cloud} makeSync={makeSync} makeDelete={makeDelete} />
    ));    
        ...
        <>
            <Container>
                ...
                <Group>
                    ...
                    <Button leftIcon={<IconCloudPlus size="1.125rem" />} onClick={openAddCloudModal}>
                        Add Cloud
                    </Button>
                </Group>
                ...
                {!cloudsLoading && cloudsError == null && clouds.length === 0 && (
                    <Text p={"xs"}>
                        You haven't added any clouds yet. Please add at least one
                        cloud.
                    </Text>
                )}
            ...
            <Modal opened={addCloudModalOpened} onClose={closeAddCloudModalIfNotInProgress} title="Cloud access data" centered>
                <LoadingOverlay visible={addCloudInProgress}/>
                <AddCloudForm makeAddCloud={makeAddCloud} onCancel={closeAddCloudModalIfNotInProgress} />
            </Modal>            
        </>
    );
}
